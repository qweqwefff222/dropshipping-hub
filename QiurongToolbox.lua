...
local function ctlParagraph(sec, o)
	...
	if statItems then
		...
		for i, s in ipairs(statItems) do
			...
			local sv = New("TextLabel", {
				...
				Size = UDim2.new(1, -20, 0, ctx.M.mobile and 16 or 20),
				Font = Enum.Font.GothamBold, TextSize = ctx.M.mobile and 14 or 18,
				TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd,
				Text = tostring(s.v or s[2] or ""), Parent = sc,
			})
	...
	local function rebuildDesc(items)
		...
		if statsGrid then
			...
			local cellH = ctx.M.mobile and 42 or 52
			statsGrid.Size = UDim2.new(1, -sp * 2, 0, rows * cellH + (rows - 1) * 8)
