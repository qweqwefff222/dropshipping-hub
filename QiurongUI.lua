--[[
  ...
  控件 18 种：toggle slider number input dropdown multi segment button keybind
             color status list progress tags table log category collapsible
  运行期读写：State.Controls[key] 的 SetValue/GetValue（dropdown 另有 SetOptions）。
  实时刷新：status/progress 的 value 传函数即自动 0.4s 轮询（窗口可见时）。
  状态存取：State:Get/Set(kind,key,v)；multi 的值是集合 {[值]=true}。
]]
--=====================================================================
